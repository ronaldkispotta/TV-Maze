...
function App() {
  ...
  const searchActor = (search) => {
    setSearch(search)
    axios
      .get(`https://api.tvmaze.com/search/people?q=${search}`)
      .then((res) => {
        console.log(res.data)
        setData(res.data)
      })
                ...
                <div key={i} className='col-xs-12 col-md-3'>
                  {data.show && (
                    <div className='movie-box'>
                      {data.show.image ? (
                        <img src={data.show.image.medium} alt='Show' />
                      ) : (
                        <img src='../images/blank.jpg' alt='Show' />
                      )}
                  ...
                  {data.person && (
                    <div className='movie-box'>
                      {data.person.image ? (
                        <img src={data.person.image.medium} alt='Person' />
                      ) : (
                        <img src='../images/blank.jpg' alt='Person' />
                      )}
